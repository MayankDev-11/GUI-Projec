...
# root.iconbitmap("C:\Users\Mayank\OneDrive\Documents\Python\Python projects\Color Theme Maker\color_wheel.ico")
...
def update_color():
    ...
    #     Display the value of the choosen color.

    color_tuple.config(text = "("+str(red_slider.get())+"),"+"("+str(green_slider.get())+"),"+"("+str(blue_slider.get())+")")
    color_hex.config(text = "#"+str(red_value+green_value+blue_value))


    # color_tuple.config(text='(' + str(red_slider.get()) + '),' + '(' +(green_slider.get()) + '),' + '(' + str(blue_slider.get()) + ')')
...
def store_color():
    ...
    new_color_hex = tkinter.Label(output_frame,text = "#"+str(red_value+green_value+blue_value))
    new_color_black_box = tkinter.Label(output_frame, bg = "black",width = 3,height = 1)
    new_color_box = tkinter.Label(output_frame,bg = "#"+str(red_value+green_value+blue_value),width = 3,height = 1)
...
#     Move the radiobutton if selected previously
...
red_button.grid(row = 2,column =0,padx = 1,pady = 1,ipadx = 38)
...
# Creating the color box and color labels
...
color_box.grid(row = 1,column = 3,columnspan = 2,padx= 35,pady = 10,ipady = 10)
...
for i in range(6):
    ...
    radio.grid(row = i , column = 0,sticky = "N")
